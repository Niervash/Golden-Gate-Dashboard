...
function loadFromStorage(): { students: Student[]; source: SpreadsheetSource | null } {
  try {
...
export const StudentProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const [lastSyncAt, setLastSyncAt] = useState<string | null>(saved.source?.importedAt ?? null);
  const [gsheetsUrl, setGsheetsUrl] = useState(saved.source?.type === "gsheets" ? saved.source.label : "");
  ...
  const applyImport = (parsed: Student[], src: SpreadsheetSource) => {
    ...
    setImportMessage(`✓ ${parsed.length} data siswa berhasil disinkronkan dari ${src.type === "gsheets" ? "Google Sheets" : src.label}`);
  };
  ...
  const importFromFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      if (parsed.length === 0) {
        ...
        setImportMessage("Tidak ada data ditemukan. Pastikan ada kolom: NIS, Nama, Kelas.");
        return;
      }
      ...
      setImportStatus("error");
      setImportMessage("Gagal membaca file. Pastikan format .xlsx atau .csv yang valid.");
    }
  }, []);
  ...
  const importFromGSheets = useCallback(async (url?: string) => {
    ...
    if (!match) {
      setImportStatus("error");
      setImportMessage("URL tidak valid. Gunakan link Google Sheets yang benar.");
      return;
    }
  ...
  const downloadTemplate = useCallback(async () => {
    ...
    const templateData = [
      { NIS: "2024001", NISN: "1234567890", "Nama Lengkap": "Contoh Nama Siswa", Kelas: "X-1", Jurusan: "MIPA", "Jenis Kelamin": "L", Status: "Aktif", "Tanggal Lahir": "2008-01-15", "Tempat Lahir": "Makassar", Alamat: "Jl. Contoh No. 1", "No Telp": "08123456789", Email: "siswa@email.com", "Nama Wali": "Nama Orang Tua", "Pekerjaan Wali": "Wiraswasta", "No Telp Wali": "08987654321" },
      { NIS: "2024002", NISN: "0987654321", "Nama Lengkap": "Contoh Siswa Kedua", Kelas: "X-2", Jurusan: "IPS", "Jenis Kelamin": "P", Status: "Aktif", "Tanggal Lahir": "2008-03-20", "Tempat Lahir": "Gowa", Alamat: "Jl. Contoh No. 2", "No Telp": "08234567890", Email: "siswa2@email.com", "Nama Wali": "Nama Wali Kedua", "Pekerjaan Wali": "PNS", "No Telp Wali": "08876543210" },
    ];
